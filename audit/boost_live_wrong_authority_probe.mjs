...
function tok(v){if(!v)return {exists:false,amount:'0'};const b=Buffer.from(v.data[0],'base64');return {exists:true,amount:b.length>=72?b.readBigUInt64LE(64).toString():'0',ownerProgram:v.owner}}
...
function summarize(r,names){const v=r.value||{},logs=v.logs||[],acs=v.accounts||[];return {slot:r.context?.slot??null,err:v.err??null,units:v.unitsConsumed,accounts:Object.fromEntries(names.map((n,i)=>[n,n==='pool'?pool(acs[i]):tok(acs[i])])),anchor:logs.filter(x=>x.includes('AnchorError')||x.includes('Error Code:')),instructions:logs.filter(x=>x.includes('Instruction:')),failed:logs.filter(x=>x.includes('failed:')),logs}}

async function main(){
 const p=pool(await info(POOL)); if(BigInt(p.virtual)!==0n)throw new Error('pool already boosted live'); if(p.quoteMint!==WSOL.toBase58())throw new Error('not WSOL');
 ...
 const initOnly=await simulate(common,watch,bh);const ini=summarize(initOnly,names);console.log('INIT_ONLY',JSON.stringify(ini,null,2));if(ini.err!==null)throw new Error('init failed');const available=BigInt(ini.accounts.vault.amount);if(available<=0n)throw new Error('init vault empty');let amount=available/100n;if(amount>1_000_000_000n)amount=1_000_000_000n;if(amount<1_000_000n)amount=available/2n;
 ...
 console.log('DECISIVE',JSON.stringify({amount:amount.toString(),correctErr:c.err,wrongErr:w.err,correctSlot:c.slot,wrongSlot:w.slot,correctVault:c.accounts.vault.amount,wrongVault:w.accounts.vault.amount,correctVirtual:c.accounts.pool.virtual,wrongVirtual:w.accounts.pool.virtual,wrongAnchor:w.anchor,wrongInstructions:w.instructions},null,2));
}
